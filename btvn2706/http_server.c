...
void handle_request(int client_socket) {
	...
	long bytes_received = recv(client_socket, request, BUFFER_SIZE, 0);
	if ( bytes_received < 0 ) {
		perror("recv");
		return;
	}
	...
	if ( strcmp(path, "/") == 0 ) {
		open_root(client_socket, "C:/Users/admin/Desktop/hustcan/ltm/btvn2706/sample_html");
	} else {
...
void open_root(int client_socket, const char *path) {
...
void send_response(int client_socket, int status_code, const char *message_body) {
	char *status_phrase;
	switch ( status_code ) {
		case 200:status_phrase = "OK";
			break;
		case 404:status_phrase = "Not Found";
			break;
		case 400:status_phrase = "Bad Request";
			break;
		case 501:status_phrase = "Internal Server Error";
			break;
